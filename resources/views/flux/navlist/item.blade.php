...
$iconClasses = Flux::classes()
    ->add($square ? 'size-15!' : 'size-10!')
    ->add('text-[#c1c89d]')
    ;

$classes = Flux::classes()
    ->add('h-12 relative flex items-center gap-3')
    ->add($square ? 'px-2.5!' : '')
    ->add('py-0 text-start w-full px-3 my-px')
    ->add('text-zinc-100 dark:text-white/80')
    ->add(match ($variant) {
        'outline' => match ($accent) {
            true => [
                'data-current:text-zinc-100 hover:data-current:text-(--color-accent-content)',
                'data-current:bg-gradient-to-l data-current:from-white/30 dark:data-current:bg-white/[7%] dark:data-current:border-transparent',
                'hover:bg-gradient-to-l hover:from-zinc-100/5 hover:text-zinc-100 dark:hover:text-white dark:hover:bg-white/[7%]',
...
<flux:button-or-link :attributes="$attributes->class($classes)" data-flux-navlist-item>
    ...
    <?php if ($slot->isNotEmpty()): ?>
        <div class="flex-1 text-lg font-medium leading-none whitespace-nowrap [[data-nav-footer]_&]:hidden [[data-nav-sidebar]_[data-nav-footer]_&]:block" data-content>{{ $slot }}</div>
    <?php endif; ?>
